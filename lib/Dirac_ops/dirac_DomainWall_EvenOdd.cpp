...
const Field Dirac_DomainWall_EvenOdd::
...
const Field Dirac_DomainWall_EvenOdd::mult(const Field& f) const{
#ifdef  IBM_BGQ_WILSON
...
const Field Dirac_DomainWall_EvenOdd::mult_dag(const Field& f) const{
...
md_force(const Field& eta,const Field& zeta) const{
  ...
  _Message(DEBUG_VERB_LEVEL, 
	   "[Timing] - Dirac_DomainWall_EvenOdd::md_force"
           << " - md_force_eo timing = "
           << timing << std::endl);
...
// OPTIMIZED LIBRARIES
void  Dirac_DomainWall_EvenOdd::
solve_eo(Field& out,const Field& in,SolverOutput& SO,int Niter,
	 double stop_cond) const{ Deo_.solve_eo_5d(out,in,SO,Niter,stop_cond);}

void Dirac_DomainWall_EvenOdd::
solve_ms_eo(vector<Field>& xq,const Field& b,SolverOutput& SO, 
	    const vector<double>& sigma,int MaxIter,double target_prec) const{
