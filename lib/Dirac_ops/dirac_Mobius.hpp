 ...
 *  @brief Definition of Mobius operator
 Time-stamp: <2013-05-21 11:53:30 noaki>
*/
...
public:
  Dirac_Mobius(const XML::node& DMnode,
    ...
    XML::read(Dnode,"mass",massd,MANDATORY);
    ...
    if(c == 0.0){
      ...
      c1_=b/c;
      c2_=-2.0*b/(c*c*(massd+4.0));
      // remove  *(mass_d +4.0)  after D_W is canonically normalized !!!!  
    }
  }
  ...
  Dirac_Mobius(const DiracWilsonLike* D,const Solver* slv,
    ...
    :Dd_(D),slv_(slv),
     c1_(b/c),c2_(-2.0*b/(c*c*(massd+4.0))){}
