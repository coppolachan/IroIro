...
void Dirac_Wilson::mult_xp(Field& fp, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    ...
    if(sf_up_[0]->on_bdry(site)){
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[0]->re_bdry(c,0,site) -sf_up_[0]->im_bdry(c,3,site);
	w1i[c] = sf_up_[0]->im_bdry(c,0,site) +sf_up_[0]->re_bdry(c,3,site);
	w2r[c] = sf_up_[0]->re_bdry(c,1,site) -sf_up_[0]->im_bdry(c,2,site);
	w2i[c] = sf_up_[0]->im_bdry(c,1,site) +sf_up_[0]->re_bdry(c,2,site);
      } 
    }else{
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[0]->re_bulk(c,0,site) -sf_up_[0]->im_bulk(c,3,site);
	w1i[c] = sf_up_[0]->im_bulk(c,0,site) +sf_up_[0]->re_bulk(c,3,site);
	w2r[c] = sf_up_[0]->re_bulk(c,1,site) -sf_up_[0]->im_bulk(c,2,site);
	w2i[c] = sf_up_[0]->im_bulk(c,1,site) +sf_up_[0]->re_bulk(c,2,site);
      }
    }
    for(int c=0; c<NC_; ++c){
      v1r[c] = 0.0; v1i[c] = 0.0; 
...
void Dirac_Wilson::mult_yp(Field& fp, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    ...
    if(sf_up_[1]->on_bdry(site)){
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[1]->re_bdry(c,0,site) +sf_up_[1]->re_bdry(c,3,site);
	w1i[c] = sf_up_[1]->im_bdry(c,0,site) +sf_up_[1]->im_bdry(c,3,site);
	w2r[c] = sf_up_[1]->re_bdry(c,1,site) -sf_up_[1]->re_bdry(c,2,site);
	w2i[c] = sf_up_[1]->im_bdry(c,1,site) -sf_up_[1]->im_bdry(c,2,site);
      }
    }else{ 
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[1]->re_bulk(c,0,site) +sf_up_[1]->re_bulk(c,3,site);
	w1i[c] = sf_up_[1]->im_bulk(c,0,site) +sf_up_[1]->im_bulk(c,3,site);
	w2r[c] = sf_up_[1]->re_bulk(c,1,site) -sf_up_[1]->re_bulk(c,2,site);
	w2i[c] = sf_up_[1]->im_bulk(c,1,site) -sf_up_[1]->im_bulk(c,2,site);
      }
    }
    for(int c=0; c<NC_; ++c){
      v1r[c] = 0.0; v1i[c] = 0.0; 
...
void Dirac_Wilson::mult_zp(Field& fp, const Field& f) const{
  ...
  for(int site=0; site <Nvol_; ++site){
    ...
    if(sf_up_[2]->on_bdry(site)){
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[2]->re_bdry(c,0,site) -sf_up_[2]->im_bdry(c,2,site);
	w1i[c] = sf_up_[2]->im_bdry(c,0,site) +sf_up_[2]->re_bdry(c,2,site);
	w2r[c] = sf_up_[2]->re_bdry(c,1,site) +sf_up_[2]->im_bdry(c,3,site);
	w2i[c] = sf_up_[2]->im_bdry(c,1,site) -sf_up_[2]->re_bdry(c,3,site);
      }
    }else{
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[2]->re_bulk(c,0,site) -sf_up_[2]->im_bulk(c,2,site);
	w1i[c] = sf_up_[2]->im_bulk(c,0,site) +sf_up_[2]->re_bulk(c,2,site);
	w2r[c] = sf_up_[2]->re_bulk(c,1,site) +sf_up_[2]->im_bulk(c,3,site);
	w2i[c] = sf_up_[2]->im_bulk(c,1,site) -sf_up_[2]->re_bulk(c,3,site);
      }
    }
    for(int c=0; c<NC_; ++c){
      v1r[c] = 0.0; v1i[c] = 0.0; 
...
void Dirac_Wilson::mult_tp(Field& fp, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    ...
    if(sf_up_[3]->on_bdry(site)){
      for (int c = 0; c < NC_; ++c){
	w1r[c] = sf_up_[3]->re_bdry(c,2,site)*2.0;
	w1i[c] = sf_up_[3]->im_bdry(c,2,site)*2.0;
	w2r[c] = sf_up_[3]->re_bdry(c,3,site)*2.0;
	w2i[c] = sf_up_[3]->im_bdry(c,3,site)*2.0;
      }
    }else{
      for(int c=0; c<NC_; ++c){
	w1r[c] = sf_up_[3]->re_bulk(c,2,site)*2.0;
	w1i[c] = sf_up_[3]->im_bulk(c,2,site)*2.0;
	w2r[c] = sf_up_[3]->re_bulk(c,3,site)*2.0;
	w2i[c] = sf_up_[3]->im_bulk(c,3,site)*2.0;
      }
    }
    for(int c=0; c<NC_; ++c){
      v1r[c] = 0.0; v1i[c] = 0.0; 
...
void Dirac_Wilson::mult_xm(Field& fm, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    for(int c1=0; c1<NC_; ++c1){
      for(int c2=0; c2<NC_; ++c2){
	U[re(c2,c1)] =  (*u_)[gf_->index_r(c1,c2,(this->*gm)(site),0)];
	U[im(c2,c1)] = -(*u_)[gf_->index_i(c1,c2,(this->*gm)(site),0)];
      }
    }
    for(int c=0; c<NC_; ++c){
...
void Dirac_Wilson::mult_ym(Field& fm, const Field& f) const{
  ...
  for(int site = 0; site<Nvol_; ++site){
    for(int c1=0; c1<NC_; ++c1){
      for(int c2=0; c2<NC_; ++c2){
	U[re(c2,c1)] =  (*u_)[gf_->index_r(c1,c2,(this->*gm)(site),1)];
	U[im(c2,c1)] = -(*u_)[gf_->index_i(c1,c2,(this->*gm)(site),1)];
      }
    }
    for(int c=0; c<NC_; ++c){
...
void Dirac_Wilson::mult_zm(Field& fm, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    for(int c1=0; c1<NC_; ++c1){
      for(int c2=0; c2<NC_; ++c2){
	U[re(c2,c1)] =  (*u_)[gf_->index_r(c1,c2,(this->*gm)(site),2)];
	U[im(c2,c1)] = -(*u_)[gf_->index_i(c1,c2,(this->*gm)(site),2)];
      }
    }
    for(int c=0; c<NC_; ++c){
...
void Dirac_Wilson::mult_tm(Field& fm, const Field& f) const{
  ...
  for(int site=0; site<Nvol_; ++site){
    for (int c1=0; c1<NC_; ++c1){
      for (int c2=0; c2<NC_; ++c2){
	U[re(c2,c1)] =  (*u_)[gf_->index_r(c1,c2,(this->*gm)(site),3)];
	U[im(c2,c1)] = -(*u_)[gf_->index_i(c1,c2,(this->*gm)(site),3)];
      }
    }
    for (int c=0; c<NC_; ++c){
