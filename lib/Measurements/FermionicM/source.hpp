...
template<typename FMT> class Source_Z2 :public Source{
private:
  const RandNum& rand_generator_;/*!< Random number generator (to be provided)*/
  Z2Type Type_;
  FMT* ff_;/*!< Field Format specifier */
  std::valarray<double> source_;/*!< Source field result */
public:
  /*! 
   * @brief Constructor
   */
  Source_Z2(const RandNum& rand, int Nvol, Z2Type Type)
   ...
   * @brief Destructor
   */
  ~Source_Z2(){ delete ff_;}

  const Field setup_source(){
    Field white_noise(ff_->size()/2);
    double cosine;
    ...
    for (int idx = 0; idx < white_noise.size(); ++idx){
      ...
      source_[2*idx] = (cosine > 0) ? 1 : ((cosine < 0) ? -1 : 0);
      source_[2*idx+1] = 0;	
      if (Type_ == ComplexZ2) {
...
template<typename FMT>
const Field Source_Z2<FMT>::mksrc(int s, int c){
  Field wns(ff_->size());
