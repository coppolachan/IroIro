...
#pragma omp parallel 
  {
    ...
    for(int mu=0; mu<NDIM_; ++mu){ /*!<gauge tr. is always done for NDIM-dims*/
      ...
      BGWilsonSU3_MatMult_NN(U1d_ptr+str2,Ge_ptr+str2,Ue_ptr+mu*NCC+str2,ns);
      BGWilsonSU3_MatEquate(Ue_ptr+mu*NCC+str2,U1d_ptr+str2,ns);

      BGWilsonSU3_MatMult_ND(U1d_ptr+str2,Uo_ptr+mu*NCC+str2,Gp_ptr+str2,ns);
      BGWilsonSU3_MatEquate(Uo_ptr+mu*NCC+str2,U1d_ptr+str2,ns);
    }
  }
...
#pragma omp parallel 
  {
    ...
    for(int mu=0; mu<NDIM_; ++mu){ /*!<gauge tr. is always done for NDIM-dims*/
      ...
      BGWilsonSU3_MatMult_ND(U1d_ptr+str2,Ue_ptr+mu*NCC+str2,Gp_ptr+str2,ns);
      BGWilsonSU3_MatEquate(Ue_ptr+mu*NCC+str2,U1d_ptr+str2,ns);

      BGWilsonSU3_MatMult_NN(U1d_ptr+str2,Go_ptr+str2,Uo_ptr+mu*NCC+str2,ns);
      BGWilsonSU3_MatEquate(Uo_ptr+mu*NCC+str2,U1d_ptr+str2,ns);
    }
  }
...
#pragma omp parallel
  {
    ...
    for(int mu=0; mu<Ndim_; ++mu){
      BGWilsonSU3_MatSub(UmU_ptr+str2,Ue_ptr+mu*NCC+str2,ns);
      shiftField_eo(Ute,Uo_ptr+mu*NCC,mu,Backward());
...
#pragma omp parallel
  {
    ...
    for(int mu=0; mu<Ndim_; ++mu){
      BGWilsonSU3_MatAdd(UpU_ptr+str2,Uo_ptr+mu*NCC+str2,ns);
...
#pragma omp parallel
  {
    ...
    for(int mu=0; mu<Ndim_; ++mu){
      BGWilsonSU3_MatSub(UmU_ptr+str2,Uo_ptr+mu*NCC+str2,ns);
      shiftField_oe(Uto,Ue_ptr+mu*NCC,mu,Backward());
...
#pragma omp parallel
  {
    ...
    for(int mu=0; mu<Ndim_; ++mu){
      BGWilsonSU3_MatAdd(W_ptr+str2,Ue_ptr+mu*NCC+str2,ns);
...
#pragma omp parallel
  {
    ...
    for(int mu=0; mu<Ndim_; ++mu){
      BGWilsonSU3_MatAdd(W_ptr+str2,Uo_ptr+mu*NCC+str2,ns);
...
void GaugeFixingStep::max_trace(double* w)const{
  ...
  std::complex<double>* pW = reinterpret_cast<std::complex<double>*>(w);
  std::complex<double> gt[4],G[9],w1,w2;
  /*
  ...
  *(pW+5) = gt[2]*w1 +gt[3]*w2;
  ...
  r1 = abs(conj(*(pW+4))+ *(pW+8));
  r2 = abs(conj(*(pW+7))- *(pW+5));
