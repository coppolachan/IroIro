...
int Test_GWrelEigen::run(){
  ...
  try{
	...
	CCIO::cout<<"CP:co:lmd=0 beacause of near 0-mode evals["<<i<<"]="
		  <<evals_[i]<<endl;
      else
	lmd = sign*sqrt(abs((evals_[i]*evals_[i] -mass*mass)/(1.0 -mass*mass)));
      ...
      CCIO::cout<<"CP:co:Pairling: "<< i <<" sign "<< sign <<endl;
      CCIO::cout<<"CP:co:Pairling: "<< i <<" beta "<< beta <<endl;
      CCIO::cout<<"CP:co:Pairling: "<< i <<" massless_eval "<< lmd <<endl;

      double fii = 2.0*beta*fmij -1.0 +2.0*beta*lmd -beta*beta*(1.0 -2.0*lmd*lmd);
      fii /= -fmij*(1.0 +2.0*beta*lmd+beta*beta)
